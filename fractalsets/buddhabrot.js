...
const makeSymmetricPlotter = (buffer, width, height, domain) => {
  return (x, y) => {
...
const postColorize = (buffer, width, height, colorfunc) => {
...
export const plotBuddhabrot = (width, height, maxIterations = 1000, accuracyFactor = 4, domain = MANDELBROT_DOMAIN, colorfunc = DEFAULT_COLOR_FUNC) => {
  const bailout = 2;
  const d = 2;
...
export const plotAntiBuddhabrot = (width, height, maxIterations = 1000, accuracyFactor = 4, domain = MANDELBROT_DOMAIN, colorfunc = DEFAULT_ANTI_COLOR_FUNC) => {
  const bailout = 2;
  const d = 2;
  const squaredBailout = bailout * bailout;
  const iterationsRe = new Float32Array(maxIterations);
