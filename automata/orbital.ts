...
import { PixelPlotter, PlotBuffer } from '../utils/types';

const OUTPUT_DIRECTORY = `${__dirname}/../output/orbital`;
mkdirs(OUTPUT_DIRECTORY);


const makeBufferPlotter = (buffer: PlotBuffer, width: number, height: number): PixelPlotter => {
...
export const generateOrbitals = async (width, height, seed = 100, nbIterations = 5000, palette = SIERRA, frameFolder = null): Promise<PlotBuffer> => {
  setRandomSeed(seed);

  const buffer: PlotBuffer = new Float32Array(width * height * 4).fill(255);
  ...
  for (let i = 0; i < nbIterations; i++) {
    ...
    if (frameFolder != null) {
      await saveImageBuffer(output, width, height, `${frameFolder}/frame-${i}.png`);
    }
  }

  // TODO refactor the code to have only 0-1 colors
  return buffer.map(x => x / 255); // normalize
};
