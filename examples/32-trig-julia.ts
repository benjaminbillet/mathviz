...
const plotSinJulia = async (c: ComplexNumber, bailout: number, maxIterations: number, domain: PlotDomain, suffix = '') => {
  const [ width, height ] = getPictureSize(size, domain);
  const configuredLemon = makeTrigJulia(c, sin, bailout, maxIterations);
  await plotFunction(`${OUTPUT_DIRECTORY}/sin-julia-c=${c.re}+${c.im}i${suffix}.png`, width, height, configuredLemon, domain, colorfunc);
};

const plotAverageStripeSinJulia = async (c: ComplexNumber, bailout: number, maxIterations: number, stripeDensity: number, domain: PlotDomain, suffix = '') => {
  const [ width, height ] = getPictureSize(size, domain);
  const configuredJulia = makeStripeAverageTrigJuliaLinear(c, sin, bailout, maxIterations, stripeDensity);
  await plotFunction(`${OUTPUT_DIRECTORY}/sin-julia-c=${c.re}+${c.im}i${suffix}-stripe.png`, width, height, configuredJulia, domain, colorfunc);
};
