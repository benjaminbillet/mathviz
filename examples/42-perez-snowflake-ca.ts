...
const buildAndPlotPerezFlakeCA = async (path: string, width: number, height: number, aliveSums: number[], iterations = 10) => {
  ...
  await plot2dHexCA(path, width, height, nextCellState, colors, iterations, initialState, 10, 3, 0);
};
