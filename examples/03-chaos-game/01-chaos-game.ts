...
const makeRestrictedPolygonRandomWalk = (polygon: Polygon, allowedJumps: number[], jumpLength = 0.5) => {
  let zn = randomComplex();
  while (withinPolygon(zn, polygon) === false) {
    zn = randomComplex();
  }
...
const plotRestrictedPolygonRandomWalk = async (polygon: Polygon, minSubsetSize = 3) => {
...
const customWalk = (polygon: Polygon, jumpLength = 0.5) => {
  let zn = randomComplex();
  while (withinPolygon(zn, polygon) === false) {
    zn = randomComplex();
  }
...
const customWalk2 = (polygon: Polygon, jumpLength = 0.5) => {
  let zn = randomComplex();
  while (withinPolygon(zn, polygon) === false) {
    zn = randomComplex();
  }
...
const customWalk3 = (polygon: Polygon) => {
  let zn = complex(1, 1);
  return () => {
...
const makeCustomPolygon = (n: number) => {
  const polygon = [];
