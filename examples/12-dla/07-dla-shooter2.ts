...
const buildAndPlotDla = async (nbParticles = 100) => {
  const path = `${OUTPUT_DIRECTORY}/dla-shoot2.png`;
  const width = 100 * maxRadius * 2;
  const height = 100 * maxRadius * 2;

  const center = complex(width/2, height/2);
  let towards = center;
  let shift = 0;
  const birthFunction = (): CustomParticle => {
    ...
    towards = affine.applyAffine2dFromMatrix(affine.combine(affine.translate(position.re, position.im), affine.rotate(shift), affine.translate(-position.re, -position.im)), center);
    shift += 0.0002;
  ...
  const moveFunction: DlaMoveFunction<CustomParticle> = (particle) => {
    ...
    // drawFilledCircle(particle.position.re, particle.position.im, 2, [0,0,0,1], plotter);
