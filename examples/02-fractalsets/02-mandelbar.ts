...
const plotMandelbar = (d: number, bailout: number, maxIterations: number, domain: PlotDomain, suffix = '') => {
  const [ width, height ] = getPictureSize(size, domain);
  const configuredMandelbar = makeMandelbar(d, bailout, maxIterations);
  plotFunction(`${OUTPUT_DIRECTORY}/mandelbar-d=${d}${suffix}.png`, width, height, configuredMandelbar, domain, colorfunc);
};

const plotContinuousMandelbar = (d: number, bailout: number, maxIterations: number, domain: PlotDomain, suffix = '') => {
  const [ width, height ] = getPictureSize(size, domain);
  const configuredMandelbar = makeContinousMandelbar(d, bailout, maxIterations);
  plotFunction(`${OUTPUT_DIRECTORY}/mandelbar-d=${d}${suffix}-continuous.png`, width, height, configuredMandelbar, domain, colorfunc);
};

const plotBitmapTrapMandelbar = (bitmapPath: string, trapSize: number, d: number, bailout: number, maxIterations: number, domain: PlotDomain, suffix = '') => {
  ...
  plotFunction(`${OUTPUT_DIRECTORY}/mandelbar-d=${d}${suffix}-trap.png`, width, height, configuredMandelbar, domain);
};
