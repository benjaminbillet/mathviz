...
import { BlendFunction } from '../../utils/types';

const sourcePath1 = `${__dirname}/../ada-big.png`;
const sourcePath2 = `${__dirname}/../clouds.png`;

const applyBlend = async (blendFunc: BlendFunction, outputPath: string) => {
  const image1 = readImage(sourcePath1);
  const image2 = readImage(sourcePath2);

  if (image1.width !== image2.width || image1.height !== image2.height) {
    throw new Error('Incompatible image size');
  }

  saveImageBuffer(blendFunc(image1.buffer, image2.buffer), image1.width, image1.height, outputPath);
};
