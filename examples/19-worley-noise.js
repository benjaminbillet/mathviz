...
const makeCenteredDistance = (size, distanceFunc) => {
  return (x1, y1, x2, y2) => {
    x1 = (x1 - size / 2);
    y1 = (y1 - size / 2);
    x2 = (x2 - size / 2);
...
const makeNormalizedDistance = (size, distanceFunc) => {
  return (x1, y1, x2, y2) => {
    x1 = x1 / size;
    y1 = y1 / size;
    x2 = x2 / size;
...
const customDistFunc = (x1, y1, x2, y2, f = 1) => euclidean2d(Math.sin(x1 * f * 2 * Math.PI), Math.sin(y1 * f * 2 * Math.PI), Math.sin(x2 * f * 2 * Math.PI), Math.sin(y2 * f * 2 * Math.PI));
...
const makeCustomDistFunc2 = (e, distFunc = euclidean2d) => {
