...
let PERMUTATIONS = new Uint8Array();

export const resetSimplexSeed = () => {
  const seeds = <number[]> shuffleArray([ 151, 160, 137, 91, 90, 15,
    ...
    PERMUTATIONS = new Uint8Array(seeds.concat(seeds));
  };
...
export const simplex = (x: number, y: number, gradients = DefaultGradients) => {
  const s = (x + y) * F;
...
export const makeSimplexNoise = (width: number, height: number, frequency = 32) => {
  frequency = clampInt(frequency, 1, Math.min(width, height) / 2);
  const noiseFunc = makeSimplexNoiseFunction(frequency);
...
export const makeSimplexNoiseFunction = (frequency = 32): NoiseFunction2D => {
  return (x, y) => simplex(x * frequency, y * frequency);
};
...
export const makeSimplexNoise2 = (width: number, height: number, preprocessFunc?: Noise2DPreProcess2D, postprocessFunc?: Noise2DPostProcess2D, gradients = DefaultGradients) => {
  ...
  const noiseFunc = makeSimplexNoiseFunction2(preprocessFunc, postprocessFunc, gradients);
  const buffer = new Float32Array(width * height * 4);
  forEachPixel(buffer, width, height, (r, g, b, a, i, j, idx) => {
    const intensity = noiseFunc(i / width, j / height);
    buffer[idx + 0] = intensity;
...
export const makeSimplexNoiseFunction2 = (preprocessFunc: Noise2DPreProcess2D, postprocessFunc: Noise2DPostProcess2D, gradients = DefaultGradients): NoiseFunction2D => {
  ...
  return (x, y) => {
    ...
    const intensity = simplex(coords[0], coords[1], gradients);
