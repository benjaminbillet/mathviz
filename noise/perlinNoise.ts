...
const quinticInterpolate = (x: number) => {
  const x3 = x * x * x;
  const x4 = x3 * x;
  const x5 = x4 * x;
...
const linearInterpolate = (a: number, b: number, g: number) => {
  return (1 - g) * a + g * b;
};
...
// output [-1, 1]
export const perlin = (x: number, y: number, gradients = DefaultGradients) => {
...
export const makePerlinNoise = (width: number, height: number, frequency = 32) => {
...
export const makePerlinNoiseFunction = (frequency = 32): NoiseFunction2D => {
...
export const makePerlinNoiseFunction2 = (preprocessFunc: Noise2DPreProcess2D, postprocessFunc: Noise2DPostProcess2D, gradients = DefaultGradients): NoiseFunction2D => {
