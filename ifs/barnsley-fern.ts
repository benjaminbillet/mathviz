...
export const makeFernIfs = (m = BARNSLEY_FERN_COEFFICIENTS, p = BARNSLEY_FERN_PROBABILITIES) => {
  ...
  const f1: Affine2D = (z) => complex(z.re * m[0]  + z.im * m[1]  + m[4],  z.re * m[2]  + z.im * m[3]  + m[5]);
  const f2: Affine2D = (z) => complex(z.re * m[6]  + z.im * m[7]  + m[10], z.re * m[8]  + z.im * m[9]  + m[11]);
  const f3: Affine2D = (z) => complex(z.re * m[12] + z.im * m[13] + m[16], z.re * m[14] + z.im * m[15] + m[17]);
  const f4: Affine2D = (z) => complex(z.re * m[18] + z.im * m[19] + m[22], z.re * m[20] + z.im * m[21] + m[23]);
