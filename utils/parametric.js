...
export const circle = (n) => { // polar to cartesian coordinates
  return complex(Math.cos(n), Math.sin(n));
};

export const astroid = (n) => {
  const sinn = Math.sin(n);
  const cosn = Math.cos(n);
  return complex(sinn * sinn * sinn, cosn * cosn * cosn);
};

export const cissoid = (n) => {
  let x = Math.sin(n);
  x = 2 * x * x;
  return complex(x, x * Math.tan(n));
};

export const kampyle = (n) => {
  const x = 1 / Math.sin(n);
  return complex(x, x * Math.tan(n));
};

export const rectangularHyperbola = (n) => {
  return complex(1 / Math.sin(n), Math.tan(n));
};

export const superformula = (n, a = 1, b = 1, m = 6, n1 = 1, n2 = 7, n3 = 8) => {
