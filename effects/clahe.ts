import { makeRegionHistogram } from '../utils/histogram';
import { ColorHistogram, SingleChannelHistogram } from '../utils/types';

const clipHistogram = (hist: ColorHistogram, clipLimit: number, redistribute = false): ColorHistogram => {
...
const computeCumulativeHistogram = (hist: ColorHistogram, scale: number): ColorHistogram => {
  return hist.map((histChannel) => {
    let sum = 0;
...
export const performClahe = (input: Float32Array, width: number, height: number, output: Float32Array, nbRegionsX = 16, nbRegionsY = 16, nbBins = 256, clipLimit = 128): Float32Array => {
